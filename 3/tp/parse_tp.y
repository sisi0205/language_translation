...
%type <num> suite funcdef if_stmt star_ELIF while_stmt for_stmt try_stmt plus_except opt_ELSE opt_FINALLY 
%type <num> with_stmt classdef 
%type <num> small_stmt star_SEMI_small_stmt simple_stmt star_COMMA_with_item with_item stmt  
%type <num> compound_stmt plus_stmt decorated
%type <num> expr_stmt print_stmt del_stmt pass_stmt flow_stmt import_stmt global_stmt exec_stmt assert_stmt
%type <num> test opt_IF_ELSE
...
file_input // Used in: start
	: star_NEWLINE_stmt ENDMARKER { output(st);}
	;
...
decorated // Used in: compound_stmt
	: decorators classdef {$$=0;}
	| decorators funcdef {$$=0;}
	;
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite
	{
	$$=$5+1;
	   std::cout<<"\(\""<<@1.first_line<<":0:"<<"  "<<"\'"<<$2<<"\'\""<<','<<$5+1<<')'<<std::endl;

	   if (final==0) try_n=except=fina_comp=0; 
	   func_comp +=complex+1-fina_comp;
	   if(@1.first_column==1) 
	   {
	   stct_out.line=@1.first_line;
	   stct_out.fun_n=$2;
	   free($2);
...
stmt // Used in: pick_NEWLINE_stmt, plus_stmt
	: simple_stmt {$$+=0; std::cout<<" simple stmt "<<$$<<"line "<<@1.first_line<<std::endl;}
	| compound_stmt {$$=$1; std::cout<<" stmt "<<$$<<"line "<<@1.first_line<<std::endl;}
	;
simple_stmt // Used in: stmt, suite
	: small_stmt star_SEMI_small_stmt SEMI NEWLINE {$$=0;}
	| small_stmt star_SEMI_small_stmt NEWLINE {$$=0;}
	;
star_SEMI_small_stmt // Used in: simple_stmt, star_SEMI_small_stmt
	: star_SEMI_small_stmt SEMI small_stmt {$$=0;}
	| %empty {$$=0;}
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist {$$=0;}
	| testlist star_EQUAL {$$=0;}
	;
...
exec_stmt // Used in: small_stmt
	: EXEC expr IN test opt_COMMA_test {$$=0;}
	| EXEC expr {$$=0;}
	;
...
compound_stmt // Used in: stmt
	: if_stmt {complex+=1; compound+=$$=$1; std::cout<<" stmt "<<$$<<"line "<<@1.first_line<<std::endl;}
	| while_stmt {complex+=1; fina_comp++; compound+=$$=$1; std::cout<<" compound_stmt "<<$$<<std::endl;}
	| for_stmt {complex+=1; fina_comp++; compound+=$$=$1;  }
	| try_stmt { complex+=1; try_n +=1; fina_comp++; compound+=$$=$1;std::cout<<" compound_stmt "<<$$<<std::endl;}
	| with_stmt { compound+=$$=$1;}
	| funcdef { compound+=$$=$1;}
	| classdef { compound+=$$=$1;}
	| decorated { compound+=$$=$1;}
	;
if_stmt // Used in: compound_stmt
	: IF test COLON suite star_ELIF ELSE COLON suite {$$=$4+$5+$8+1;std::cout<<" if "<<$$<<std::endl;}
	| IF test COLON suite star_ELIF {$$=$4+$5+1; std::cout<<" if "<<$$<<std::endl;}
	;
...
while_stmt // Used in: compound_stmt
	: WHILE test COLON suite ELSE COLON suite {$$=$7+$4 +1; }
	| WHILE test COLON suite {$$=$4+1; std::cout<<"while "<<$$<<std::endl;}
	;
for_stmt // Used in: compound_stmt
	: FOR exprlist IN testlist COLON suite ELSE COLON suite {$$=$9+1+$6;}
	| FOR exprlist IN testlist COLON suite {$$=$6+1;}
	;
try_stmt // Used in: compound_stmt
	: TRY COLON suite plus_except opt_ELSE opt_FINALLY {if($6!=1) $$=$3+$4+1; else $$=0; std::cout<<" try "<<$$<< "finally "<<$6<<std::endl;}
	| TRY COLON suite FINALLY COLON suite {$$=0;std::cout<<" try "<<$$<<std::endl;}
	;
plus_except // Used in: try_stmt, plus_except 
	: plus_except except_clause COLON suite {$$=$1+$4+1;complex+=1;except+=1;fina_comp++;}
	| except_clause COLON suite {$$=$3+1; std::cout<<" except "<<$$<<std::endl;}
	;
...
with_stmt // Used in: compound_stmt
	: WITH with_item star_COMMA_with_item COLON suite {$$=0;}
	;
star_COMMA_with_item // Used in: with_stmt, star_COMMA_with_item
	: star_COMMA_with_item COMMA with_item {$$=0;}
	| %empty {$$=0;}
	;
...
suite // Used in: funcdef, if_stmt, star_ELIF, while_stmt, for_stmt, try_stmt, plus_except, opt_ELSE, opt_FINALLY, with_stmt, classdef
	: simple_stmt {$$=$1;std::cout<<" simple_stat "<<$$<< "line  "<<@1.first_line<<std::endl;}
	| NEWLINE INDENT plus_stmt DEDENT {$$=$3; std::cout<<"suit"<< $$ <<"line  "<<@1.first_line<<std::endl;}
	;
plus_stmt // Used in: suite, plus_stmt
	: plus_stmt stmt { $$=$1+$2; std::cout<<" plus_ stmt"<<$$<<"line  "<<@1.first_line<<std::endl;}
	| stmt {$$=$1; std::cout<<" plus_ stmt"<<$$<<"line  "<<@1.first_line<<std::endl;}
	;
...
classdef // Used in: decorated, compound_stmt
	: CLASS NAME LPAR opt_testlist RPAR COLON suite {$$=0;}
	| CLASS NAME COLON suite {$$=0;}
	;
