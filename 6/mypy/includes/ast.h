...
public:
  ...
  virtual void Insert( Node* node) { stmts.push_back(node);}
...
public:
  ...
  void Insert(Node* node) { parameters.push_back(node);}
  void FInsert(Node* node) {parameters.insert(parameters.begin(),node );}
  virtual const Literal* eval()const;
...
public:
  ...
  void Insert(Node* node) { argument.push_back(node);}
  void InsertVector(Node*);
  virtual const Literal* eval() const;
  std::vector<Node*>  getArg() const {return argument; }
...
public:
  FuncNode(const std::string fn, Node* pa, Node* su) :Node(), FuncName(fn),parameter(pa), suite(su) 
  {
...
public:
  CallNode(const std::string fn, Node* arg ) :Node(), ident(fn),argument(arg)
  {
  //TableManager::getInstance().insert(fn,su);
  }
  virtual const Literal* eval() const ;
